 ...
 */
class InlineColorPickerElement extends LitElement {
	...
	render() {
		...
		return html`
			<div class="gaa-inline-color-picker-interactive">
				<div
					class="gaa-inline-color-picker-color-space"
					tabindex="0"
				...
				>
					<div
						class="gaa-inline-color-picker-color-space-thumb"
						style=${styleMap({
				...
				<div class="flexrow gap-05rem">
					<div class="flexcol gap-05rem">
						<input
							type="range"
							class="gaa-inline-color-picker-hue-range"
							min="0"
							max="359"
							step="1"
							.value=${hsva.h}
						...
						<input
							type="range"
							class="gaa-inline-color-picker-alpha-range"
							min="0"
							max="100"
							step="1"
							.value=${hsva.a}
			...
			<div class="gaa-inline-color-picker-inputs">
				...
				<div style="margin-bottom: 0.5rem;">
					<label for=${`gaa-colorpicker-${this.#id}-hex`}>Hex</label>
					<input
						type="text"
						id=${`gaa-colorpicker-${this.#id}-hex`}
						maxlength="9"
				...
				${["r", "g", "b"].map(k => html`
					<div>
						<label for=${`gaa-colorpicker-${this.#id}-${k}`}>${k.toUpperCase()}</label>
						<input
							type="number"
							id=${`gaa-colorpicker-${this.#id}-${k}`}
							min="0"
							max="255"
							step="1"
				...
				<div>
					<label for=${`gaa-colorpicker-${this.#id}-a`}>A</label>
					<input
						type="number"
						id=${`gaa-colorpicker-${this.#id}-a`}
						min="0"
						max="100"
						step="1"
...
customElements.define(elementName, InlineColorPickerElement);
